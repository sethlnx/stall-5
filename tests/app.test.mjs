...
test('app phases, coverage controls, fake, autoplay and default clock work together', async () => {
  ...
  frames(20);
  assert(game.frame > decisionFrame, 'release decision also keeps moving');
  const fakeFrame = game.frame;
  click('fake');
  assert.equal(game.frame, fakeFrame, 'faking resumes from the current frame');
  ...
  assert.equal(game.phase, 'defense');
  frames(250);
  assert.equal(game.phase, 'offense', 'a live decision naturally reaches the next turn');
  assert.equal(node('defense-controls').hidden, true);
});
